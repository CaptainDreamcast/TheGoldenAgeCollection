...
static void loadChangeGroup() {
	...
	if (!strcmp("code", type)) {
		changeHackProjectCode(delta);
	} else if (!strcmp("design", type)) {
		changeHackProjectDesign(delta);
	} else if (!strcmp("marketing", type)) {
		changeHackProjectMarketing(delta);
	}
	else if (!strcmp("time", type)) {
		changeHackProjectTime(delta);
	}
	else {
...
static void goToHack(void* tCaller) {
	(void)tCaller;
	changeHackProjectTime(1);
	if (isHackTimeOver()) {
		setCurrentHackStoryDefinitionFile("assets/misc/hack/story/ENDING.def");
...
Screen HackStoryScreen = {
	.mLoad = loadStoryScreen,
	.mUpdate = updateStoryScreen,
};


void setCurrentHackStoryDefinitionFile(char* tPath) {
