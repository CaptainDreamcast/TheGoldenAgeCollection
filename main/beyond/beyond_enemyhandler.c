...
static void enemyHitCB(void* tCaller, void* tCollisionData) {
	...
	if (collisionData->mCollisionList == getBeyondPlayerCollisionList()) return;
...
static void addSingleEnemy(StageEnemy* tEnemy, int i) {
	...
	e->mCollisionData.mCollisionList = getBeyondEnemyCollisionList();
	e->mCollisionData.mIsItem = 0;
	setMugenAnimationCollisionActive(e->mAnimationID, getBeyondEnemyCollisionList(), enemyHitCB, e, &e->mCollisionData);
...
static void updateEnemyShot(ActiveEnemy* e) {
	if (handleDurationAndCheckIfOver(&e->mShotNow, e->mShotFrequency)) {
		e->mShotNow = 0;
		addBeyondShot(e->mShotType, getBeyondEnemyShotCollisionList(), *getHandledPhysicsPositionReference(e->mPhysicsID));
	}
}
...
ActorBlueprint BeyondEnemyHandler = {
